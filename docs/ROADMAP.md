...
| **D6** | Polish + installer: electron-builder NSIS installer (single-file, one-click, desktop shortcut), full **brand identity + logo system** (`docs/BRAND.md`, `desktop/build/`), close-to-tray system tray, read-only GitHub-releases update check in Settings, **UX audit vs `.impeccable` standards** | **all in repo, builds + 68 host tests green; silent auto-update pending (blocked on code signing — feasibility mapped in `docs/DESKTOP.md`)** |
