...
| **D6** | Polish + installer: electron-builder NSIS installer (single-file, one-click, desktop shortcut), full **brand identity + logo system** (`docs/BRAND.md`, `desktop/build/`), close-to-tray system tray, read-only GitHub-releases update check in Settings | **installer + logo system + tray + update check in repo, builds + 66 host tests green; silent auto-update pending (unsigned build), UX audit pending** |
